...
def viewbook(request, bookId):
    ...
    book1 = {'id': 123, 'title': 'Continuous Delivery',
             'author': 'J. Humble and D. Farley'}
    book2 = {'id': 456, 'title': 'Secrets of Reverse Engineering',
             'author': 'E. Eilam'}
    targetBook = None
    if book1['id'] == bookId:
        targetBook = book1
...
def __getBooksList():
    ...
    book2 = {'id': 56788765, 'title': 'Reverse Engineering', 'author': 'E. Eilam'}
    book3 = {'id': 43211234, 'title': 'Machine Learning Book', 'author': 'Andriy'}
...
def simple_query(request):
    # to check if it is there y, you can put the insted of and
...
def lookup_query(request):
    mybooks = Book.objects.filter(author__isnull=False).filter(
        title__icontains='and').filter(edition__gte=2).exclude(price__lte=100)[:10]
